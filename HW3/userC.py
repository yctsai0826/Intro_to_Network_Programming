...
class GamePlayer:
    def __init__(self, host="140.113.235.154"):
        ...
        self.invitations = []
        self.games = {}
        self.stop_broadcast_thread = False  # 用於控制廣播執行緒
        ...
        try:
            self.broadcast_socket.connect((LobbyServer, BroadcastPort))
            print("\n!!! Connected to the Broadcast Server successfully. !!!\n")
        except socket.error as e:
            print(f"\n!!! Failed to connect to the Broadcast Server: {e} !!!\n")
            sys.exit(1)
    ...
    def listen_for_broadcast(self):
        try:
            while not self.stop_broadcast_thread:
                self.broadcast_socket.settimeout(1)
                try:
                    msg = self.broadcast_socket.recv(1024).decode()
                    if msg:
                        if msg.startswith("BROADCAST"):
                            print(f"\n\n·°∴ ☆．．·°．·°∴ ☆．．[Broadcast: {msg}]．·°∴ ☆．．·°．·°∴ ☆．．\n")
                        elif msg.startswith("Invited to"):
                            print(f"\n\n✧*｡˚✧*｡＊.°·˚✧*｡＊.°·˚[Invitation: {msg}]✧*｡˚✧*｡＊.°·˚✧*｡＊.°·˚\n")
                            self.receive_invitation(msg)
    ...
    def download_game_script(self, game_name):
        ...
        print("-----------------------------------------------------------------------\n")
        print(f"Downloading game script: {game_name}.py\n")
    ...
    def run_game_script(self, script_path, is_host, game_ip, game_port):
        ...
        print(f"\n★**-.,_,.-**`*-[Running game script: {script_path}].-*-*`'`*★-.,_,.-*`\n\n")
    ...
    def handle_exit(self, signum, frame):
        ...
        time.sleep(0.1)
        self.client.close()
        self.stop_broadcast_thread = True
    ...
    def login(self):
        ...
        while True:
            ...
            if response == "SUCCESS Logged in":
                self.broadcast_socket.sendall(f"REGISTER_BC {username}".encode())
                print("\n!!! Login successful !!!\n")
                print("-----------------------------------------------------------------------")
                self.list_rooms()
                return
    ...
    def create_room(self):
        ...
        if response == "SUCCESS Room created" and public == "1":
            ...
            print("-----------------------------------------------------------------------\n")
            leaveroom = input("Do you want to leave the room? (y/n)")
            if leaveroom == "y":
                self.send_message("LEAVE_ROOM")
                return
            else:
                self.send_message("STAY_ROOM")
            ...
            if response == "Request for IP and PORT":
                self.send_message(f"{GameIP} {GamePort} {game_type}")
                time.sleep(0.1)
            ...
            while True:
                while True:
                    command = input("Your choice? (1. send invitation 2. list idle players 3. leave): ")
                    if command in ["1", "2", "3"]:
                        break
                    else:
                        print("\n!!! Invalid input. Please try again. !!!\n")
                ...
                if (command == "1"):
                    ...
                    if response == f"Invited {clientname}":
                        ...
                        cmd = input("Sending invitation to other player?(y/n)\n")
                        if cmd == "y":
                            self.send_message("SEND_INVITATION_AGAIN")
                            time.sleep(0.1)
                            continue
    ...
    def accept_invitation(self, room):
        try:
            ...
            if acc == "y":
                ...
                time.sleep(0.2)
                self.invitations.remove(room)
                print(f"\n!!! Rejected invitation to room {room}. !!!\n")
        except Exception as e:
            print(f"Error while receiving invitation: {e}")
        finally:
            # 清空緩衝區
